...
# Stacks a parar en orden inverso al arranque
STACKS=(
  "monitoreo"
  "portainer"
  "heimdall"
...
print_header() {
  ...
  echo -e "${BRed}║          ⏹️   STOP ALL  — project: util                  ║${Color_Off}"
  echo -e "${BRed}╚══════════════════════════════════════════════════════════╝${Color_Off}"
...
# ==============================================================================
# 📝 Función: stop_stack
# Descripción: Para un stack de Docker Compose dado su nombre de carpeta
# Parámetros:
...
stop_stack() {
  ...
  print_separator
  msg_time "⏹️  Parando stack: ${stack}" "INFO"
  ...
  # Mostrar contenedores que van a pararse
  local running
  running=$(docker compose ps --services --filter "status=running" 2>/dev/null)
  if [ -z "$running" ]; then
    msg "   ℹ️  Stack [${stack}] ya estaba parado." "WARNING"
    cd "$ROOT_PATH"
    return 0
  fi

  if docker compose down 2>&1; then
    msg_time "   ✅ Stack [${stack}] parado correctamente." "SUCCESS"
  else
    msg_time "   ❌ Error al parar el stack [${stack}]." "ERROR"
    cd "$ROOT_PATH"
    return 1
  fi
...
# ==============================================================================
# 📝 Función: show_status
# Descripción: Muestra el estado final — no debe quedar nada corriendo
# ==============================================================================
show_status() {
  print_separator
  ...
  local running
  running=$(docker ps --filter "label=project=util" --format "{{.Names}}")

  if [ -z "$running" ]; then
    msg "   ✅ No hay contenedores util en ejecución." "SUCCESS"
  else
    msg "   ⚠️  Todavía hay contenedores corriendo:" "WARNING"
    docker ps --filter "label=project=util" \
      --format "  ${Yellow}▸${Color_Off} {{.Names}}\t{{.Status}}" \
    | column -t
  fi
  echo ""
}
...
if [ "$ERRORS" -eq 0 ]; then
  ...
  echo ""
  echo -e "${Color_Off}"
  msg_time "🛑 Todos los stacks fueron parados correctamente." "SUCCESS"
else
  echo ""
