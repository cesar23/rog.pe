...
if (!function_exists('solu_log')) {
  ...
  function solu_log($message, string $level = 'info')
  {
    ...
    // Rotación de logs si superan 5MB
    solu_rotate_log_if_needed($log_file);
    if ($level == 'error') {
      solu_rotate_log_if_needed($log_file_error);
    }
    ...
    if ($level == 'error') {
      ...
      file_put_contents($log_file_error, $log_message, FILE_APPEND | LOCK_EX);
    }
  }
}

if (!function_exists('solu_rotate_log_if_needed')) {
  /**
   ...
   */
  function solu_rotate_log_if_needed($log_file, $max_size = 5242880) {
    if (file_exists($log_file) && filesize($log_file) > $max_size) {
      $backup_file = $log_file . '.' . date('Y-m-d-H-i-s');
      rename($log_file, $backup_file);
      
      // Mantener solo los últimos 5 archivos de backup
      solu_cleanup_old_logs($log_file);
    }
  }
}

if (!function_exists('solu_cleanup_old_logs')) {
  /**
   * Limpia archivos de log antiguos manteniendo solo los últimos 5
   *
   * @param string $log_file Ruta base del archivo de log
   */
  function solu_cleanup_old_logs($log_file, $keep_count = 5) {
    ...
    if ($backup_files && count($backup_files) > $keep_count) {
      ...
      for ($i = $keep_count; $i < count($backup_files); $i++) {
