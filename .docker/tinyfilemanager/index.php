...
$CONFIG = '{"lang":"es","error_reporting":true,"show_hidden":true,"hide_Cols":false,"theme":"dark"}';

/**
 * H3K ~ Solu File Manager V2.6
...
//Application Title
define('APP_TITLE', 'Solu File Manager');
...
// Sobreescribir la propiedad de fichero
$override = array(
    'active' => 0, //si queremos  activar
    'user_name' => 'nobody',
    'user_group' => 'nogroup',
    'user_id' => 65534,
    'group_id' => 65534
...
$default_timezone = 'America/Lima'; // UTC
                ...
                <div class="row justify-content-md-center align-content-center h-100vh">
                    <div class="card-wrapper">
                        <div class="card fat" data-bs-theme="<?php echo FM_THEME; ?>">
                            <div class="card-body">
                                <form class="form-signin" action="" method="post" autocomplete="off">
                                    <div class="mb-6">
                                        <div class="brand">
                                            <img src="https://cesar23.github.io/cdn_webs/iconos_png/logo_solucionesv3_701x141.png" alt="">
                                        </div>
                        ...
                        <div class="footer text-center">
                            &mdash;&mdash; &copy;
                            <a href="https://solucionessystem.com" target="_blank" class="text-decoration-none text-muted" data-version="<?php echo VERSION; ?>">solucionessystem.com</a> &mdash;&mdash;
                        </div>
                    </div>
                </div>
            </div>
        ...
        if ($type == "file") {
            if (!file_exists($path . '/' . $new)) {
                if (fm_is_valid_ext($new)) {
                    ...
                    // Establecer permisos nobody:nogroup para el archivo creado
                    fm_set_nobody_permissions($path . '/' . $new);
            ...
            if (fm_mkdir($path . '/' . $new, false) === true) {
                // Establecer permisos nobody:nogroup para la carpeta creada
                fm_set_nobody_permissions($path . '/' . $new);
        ...
        if (empty($f['file']['error']) && !empty($tmp_name) && $tmp_name != 'none' && $isFileAllowed) {
            if ($chunkTotal) {
                ...
                if ($chunkIndex == $chunkTotal - 1) {
                    ...
                    // Establecer permisos nobody:nogroup para el archivo subido
                    fm_set_nobody_permissions($fullPathTarget);
                }
            } else if (move_uploaded_file($tmp_name, $fullPath)) {
                // Be sure that the file has been uploaded
                if (file_exists($fullPath)) {
                    // Establecer permisos nobody:nogroup para el archivo subido
                    fm_set_nobody_permissions($fullPath);
                    $response = array(
                        'status'    => 'success',
                        'info' => "file upload successful"
                    );
...
if (isset($_POST['group'], $_POST['token']) && (isset($_POST['zip']) || isset($_POST['tar'])) && !FM_READONLY) {
    ...
    if (!empty($files)) {
        ...
        if ($res) {
            // Establecer permisos nobody:nogroup para el archivo creado
            fm_set_nobody_permissions($path . '/' . $zipname);
...
if (isset($_POST['unzip'], $_POST['token']) && !FM_READONLY) {
    ...
    if ($isValid) {
        ...
        if ($res) {
            // Establecer permisos nobody:nogroup para todos los archivos extraídos
            if (isset($tofolder) && $tofolder != '') {
                // Si se extrajo en una carpeta específica, establecer permisos recursivamente en esa carpeta
                fm_set_nobody_permissions_recursive($path);
            } else {
                // Si se extrajo en el directorio actual, establecer permisos recursivamente en todo el directorio
                fm_set_nobody_permissions_recursive($path);
            }
...
if (is_array($objects) && fm_is_exclude_items($current_path, $path)) {
    foreach ($objects as $file) {
        ...
        $new_path = $path . '/' . $file;
        // Verificar si es un enlace simbólico primero
        if (is_link($new_path) && fm_is_exclude_items($file, $new_path)) {
            // Los enlaces simbólicos se clasifican según su destino
            if (@is_dir($new_path)) {
                $folders[] = $file;
            } else {
                $files[] = $file;
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-xs-12 col-sm-6">
                        <p>
                        <h3><a href="#" target="_blank" class="app-v-title"> Solu File Manager <?php echo VERSION; ?></a></h3>
                        </p>
                        <p>Author: Cesar Auris</p>
                        <p>Mail Us: <a href="mailto:perucaos@gmail.com">perucaos@gmail.com</a> </p>
                    </div>
                    <div class="col-xs-12 col-sm-6">
                        <div class="card">
...
<form action="" method="post" class="pt-3">
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-hover table-sm" id="main-table" data-bs-theme="<?php echo FM_THEME; ?>">
            ...
            foreach ($folders as $f) {
                ...
                // Manejo seguro de metadatos para enlaces simbólicos
                $file_path = $path . '/' . $f;
                $modif_raw = @filemtime($file_path);
                if ($modif_raw === false) {
                    $modif_raw = time(); // Usar tiempo actual si falla
                }
                $modif = date(FM_DATETIME_FORMAT, $modif_raw);
                $date_sorting = strtotime(date("F d Y H:i:s.", $modif_raw));
                $filesize_raw = "";
                $filesize = lng('Folder');
                $perms_raw = @fileperms($file_path);
                $perms = $perms_raw !== false ? substr(decoct($perms_raw), -4) : '0000';
                $owner = array('name' => '?'); 
                $group = array('name' => '?');
                if (function_exists('posix_getpwuid') && function_exists('posix_getgrgid')) {
                    try {
                        ...
                        // Usar la función mejorada para obtener información del grupo
                        if ($group_id !== false) {
                            $group = fm_get_group_info($group_id);
                        }
                    } catch (Exception $e) {
            ...
            foreach ($files as $f) {
                ...
                // Manejo seguro de metadatos para archivos (incluyendo enlaces simbólicos)
                $file_path = $path . '/' . $f;
                $modif_raw = @filemtime($file_path);
                if ($modif_raw === false) {
                    $modif_raw = time(); // Usar tiempo actual si falla
    ...
    <div class="row">
        <?php if (!FM_READONLY): ?>
            ...
            <div class="col-3 d-none d-sm-block"><span class="float-right text-muted">Solu File Manager <?php echo VERSION; ?></span><br><a href="https://solucionessystem.com" target="_blank" class="float-right text-muted">www.solucionessystem.com</a></div>
        <?php else: ?>
            <div class="col-12"><span class="float-right text-muted">Solu File Manager <?php echo VERSION; ?></span><br><a href="https://solucionessystem.com" target="_blank" class="float-right text-muted">www.solucionessystem.com</a></div>
        <?php endif; ?>
    </div>
</form>
...
function fm_copy($f1, $f2, $upd)
{
    ...
    if ($ok) {
        touch($f2, $time1);
        // Establecer permisos nobody:nogroup para el archivo copiado
        fm_set_nobody_permissions($f2);
    }
    return $ok;
}
...
/**
 * Función para establecer permisos de propietario y grupo a nobody:nogroup
 * @param string $path Ruta del archivo o carpeta
 * @return bool True si se establecieron correctamente, false en caso contrario
 * @example fm_set_nobody_permissions('/path/to/file.txt');
 */
function fm_set_nobody_permissions($path)
{
    global $override;
    
    // Saltar si no configuramos la sobreescritura de dueño
    ...
    // Establecer propietario y grupo usando los valores del array
    $result = chown($path, $override['user_id']) && chgrp($path, $override['group_id']);
    
    // Log para debugging
    if (!$result) {
        error_log("No se pudo establecer permisos {$override['user_name']}:{$override['user_group']} para: $path");
    }
...
/**
 * Función para establecer permisos nobody:nogroup recursivamente
 * @param string $path Ruta del directorio
 * @return bool True si se establecieron correctamente, false en caso contrario
 * @example fm_set_nobody_permissions_recursive('/path/to/directory');
 */
function fm_set_nobody_permissions_recursive($path)
{
    if (!file_exists($path)) {
        return false;
    }
    ...
    if (!is_dir($path)) {
        return fm_set_nobody_permissions($path);
    }
    
    $result = true;
    
    try {
        // Establecer permisos para el directorio actual
        $result &= fm_set_nobody_permissions($path);
        
        // Procesar recursivamente todos los archivos y subdirectorios
        ...
        foreach ($iterator as $item) {
            if ($item->isFile() || $item->isDir()) {
                $result &= fm_set_nobody_permissions($item->getPathname());
            }
        }
    } catch (Exception $e) {
        // Si hay error con el iterator, usar método alternativo
        error_log("Error en fm_set_nobody_permissions_recursive: " . $e->getMessage());
        $result = fm_set_nobody_permissions_recursive_alternative($path);
    }
...
/**
 * Función alternativa para establecer permisos nobody:nogroup recursivamente
 * Usa métodos más básicos cuando el iterator falla
 * @param string $path Ruta del directorio
 * @return bool True si se establecieron correctamente, false en caso contrario
 */
function fm_set_nobody_permissions_recursive_alternative($path)
{
    if (!file_exists($path)) {
        return false;
    }
    ...
    // Establecer permisos para el elemento actual
    $result &= fm_set_nobody_permissions($path);
    
    if (is_dir($path)) {
        // Leer directorio y procesar recursivamente
        $files = scandir($path);
        foreach ($files as $file) {
            if ($file != '.' && $file != '..') {
                $full_path = $path . '/' . $file;
                $result &= fm_set_nobody_permissions_recursive_alternative($full_path);
            }
        }
    }
...
/**
 * Función mejorada para obtener información del grupo
 * Maneja casos donde posix_getgrgid() falla en contenedores Docker
 * @param int $group_id ID del grupo
 * @return array Información del grupo
 * @example $group_info = fm_get_group_info(1000);
 */
function fm_get_group_info($group_id)
{
    if($group_id===65534){
        return array(
            'name' => 'nogroup',
    ...
    // Intentar primero con posix_getgrgid
    ...
    // Si posix_getgrgid falla, intentar leer /etc/group directamente
    if (file_exists('/etc/group')) {
        $groups = file('/etc/group', FILE_IGNORE_NEW_LINES);
        foreach ($groups as $line) {
            $parts = explode(':', $line);
            if (count($parts) >= 3 && $parts[2] == $group_id) {
                return array(
                    'name' => $parts[0],
                    'gid' => (int)$parts[2],
                    'members' => isset($parts[3]) ? explode(',', $parts[3]) : array()
                );
            }
        }
    }
    
    // Si todo falla, intentar con grupos conocidos
    $known_groups = array(
        0 => 'root',
        1 => 'daemon',
        2 => 'bin',
    ...
    if (isset($known_groups[$group_id])) {
        return array(
            'name' => $known_groups[$group_id],
            'gid' => $group_id,
            'members' => array()
        );
    }
    
    // Para contenedores Docker: mapear GIDs no encontrados a grupos apropiados
    // Si el GID es alto (típico de usuarios del host), mapear a nogroup
    if ($group_id >= 1000 && $group_id < 65533) {
        return array(
            'name' => 'nogroup',
            'gid' => $group_id,
            'members' => array()
    ...
    // Último recurso: devolver información básica
    return array(
        'name' => 'unknown',
        'gid' => $group_id,
        'members' => array()
...
class FM_Config
{
    ...
    function __construct()
    {
        ...
        if (strlen($CONFIG)) {
            ...
            $msg = 'Solu File Manager<br>Error: Cannot load configuration';
            if (substr($fm_url, -1) == '/') {
                $fm_url = rtrim($fm_url, '/');
                $msg .= '<br>';
...
?>
    ...
    <head>
        ...
        <meta name="description" content="Web based File Manager in PHP, Manage your files efficiently and easily with Solu File Manager">
...
<?php
    ...
    function lng($txt)
    {
        ...
        // English Language
        $tr['en']['AppName']        = 'Solu File Manager';
